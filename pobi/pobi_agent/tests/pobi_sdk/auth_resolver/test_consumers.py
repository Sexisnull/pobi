...
from pobi_agent.auth_resolver.auth_resolver import CookieRecord, StorageSnapshot, AuthContext
from pobi_agent.tools.browser import run_browser_steps_tool
...
class TestBrowserRunStepsToolAuthProfile:
    ...
    async def test_missing_session_id_returns_structured_error(
        self, isolated_pobi_root: Path, patched_browser_session: dict[str, Any]
    ) -> None:
        deps = SimpleNamespace(
            target="http://localhost:8080",
            agent_id="aid",
    ...
    async def test_existing_profile_loads_state_into_browser(
        ...
        target = "http://localhost:8080"
        _seed_auth_context(isolated_pobi_root, target, "aid", "sid", "default")
