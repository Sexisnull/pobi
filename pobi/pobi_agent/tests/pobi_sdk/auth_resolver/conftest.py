...
real ``~/.pobi`` tree. The patch is applied to **both** ``constants`` and
``auth_resolver.auth_resolver`` because the latter does
``from pobi_agent.constants import DEADEND_AGENTS_PATH`` at import time.
"""
...
import pobi_agent.auth_resolver.auth_resolver as auth_resolver_module
import pobi_agent.constants as constants_module


@pytest.fixture
def isolated_pobi_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
