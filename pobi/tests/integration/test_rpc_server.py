...
async def rpc_server_process(tmp_path):
    ...
    # The server is run as: python -m pobi.jsonrpc_server
    # Test file is at: pobi/tests/integration/test_rpc_server.py
    # So we need to go up to pobi/ directory (3 levels up from test file)
    test_file = Path(__file__)
    pobi_dir = test_file.parent.parent.parent  # pobi/
    
    server_module = "pobi.jsonrpc_server"
    
    # Start the server process using asyncio
    # Run from pobi directory to avoid nested workspace issues
    # The pobi directory has its own pyproject.toml, so uv run should work from there
