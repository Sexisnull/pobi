...
# This script should be placed next to the 'pobi' binary after building
...
if [ "$(uname)" = "Darwin" ] && [ -f "${SCRIPT_DIR}/pobi" ]; then
    # Remove existing (possibly broken) signature; ignore errors if not signed
    codesign --remove-signature "${SCRIPT_DIR}/pobi" 2>/dev/null || true
    # Ad-hoc sign (no certificate)
    codesign -s - --force --deep "${SCRIPT_DIR}/pobi" 2>/dev/null || true
    # Clear quarantine attributes if any
    xattr -c "${SCRIPT_DIR}/pobi" 2>/dev/null || true
fi

exec "${SCRIPT_DIR}/pobi" "$@"
