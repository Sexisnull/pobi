...
from pobi_agent import Config, init_rag_session_manager, sandbox_setup, ModelRegistry
from pobi_agent.tools.browser_automation import cleanup_playwright_sessions
from pobi_eval.eval import EvalMetadata, eval_pobi_agent
...
async def eval_interface(
    ...
    Raises:
        RuntimeError: If no language model is configured in the model registry.
            Suggests running `pobi init` to configure models.
    ...
    if not model_registry.has_any_model():
        raise RuntimeError(f"No LM model configured. You can run `pobi init` to \
