...
from pobi_agent import ModelRegistry
import docker
...
from pobi_agent.rag.session_manager import RagSessionManager
from pobi_agent.tools.browser_automation import PlaywrightRequester
...
class ComponentManager:
    ...
    def get_model(self, provider: str | None = None, model_name: str | None = None):
        ...
        if not self.model_registry.has_any_model():
            raise RuntimeError(
                "No LLM model configured. Run `pobi init` to initialize the model configuration."
            )
