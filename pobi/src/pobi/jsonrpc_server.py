...
from pobi.jsonrpc.event_bus import EventBus
from pobi.jsonrpc.hooks_adapter import EventBusHooksAdapter
...
def main(
    ...
    if _phoenix_otel_enabled():
        ...
        project_name = os.getenv("PHOENIX_PROJECT_NAME", "pobi")
    ...
    server.add_dependency("pobi_agent_refs", pobi_agent_refs)
    ...
    async def instantiate_agent(
        ...
        pobi_agent.set_approval_callback(approval_callback)
        pobi_agent.target = target
        pobi_agent_refs.update({str(agent_id): pobi_agent})
        ...
        try:
            ...
            pobi_agent_refs.pop(str(agent_id), None)
    ...
    async def interrupt_agent(
        ...
        if not agent_id:
            ...
            return
        agent = pobi_agent_refs.get(agent_id)
        if agent is None:
            yield {
                "phase": "error",
                "data": {
    ...
    async def run_agent_recursive(
        ...
        try:
            ...
            pobi_agent = pobi_agent_refs.get(agent_id)
            if pobi_agent is None:
                yield {
                    "phase": "error",
                    "data": {
                        "message": f"Agent with id {agent_id} not found",
            ...
            pobi_agent.reset_workflow_state()
            ...
            async for item in pobi_agent.threat_model_stream(task=prompt):
            ...
            async for item in pobi_agent.start_testing_stream(
                task=prompt,
    ...
    async def run_agent_supervisor(
        ...
        try:
            ...
            task_registry.reset_agent(str(agent_id))
            pobi_agent.reset_workflow_state()
            async for item in pobi_agent.start_supervisor(task=prompt):
