...
from pobi_agent.constants import CACHE_METRICS_PATH
from pobi_agent.utils.network import deterministic_session_id
from pobi_eval.metrics import (
...
async def eval_pobi_agent(
    ...
    pobi_agent.challenge_name = eval_metadata.name

    if with_code_indexing:
        pobi_agent.init_webtarget_indexer(target_host)
        web_resources_crawler = await pobi_agent.crawl_target()
    ...
    solved = False
    plan, threat_model_data, validation_token = await pobi_agent.threat_model(task=prompt)
    ...
    if not pobi_agent.goal_achieved:
        ...
        task_node, validation_token = await pobi_agent.run_exploitation(
    ...
    metrics_md_path = CACHE_METRICS_PATH / f"pobi_metrics_{timestamp}.md"
    metrics_json_path = CACHE_METRICS_PATH / f"pobi_metrics_{timestamp}.json"

    metrics_md_path.write_text(metrics_md, encoding="utf-8")
    metrics_json_path.write_text(metrics_json, encoding="utf-8")

    print(f"Pobi metrics summary written to {metrics_md_path}")
    print(f"Pobi metrics JSON written to {metrics_json_path}")
    print(metrics_md)
