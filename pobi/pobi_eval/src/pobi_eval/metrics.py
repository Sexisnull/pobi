...
if TYPE_CHECKING:
    from pobi_agent.agents.factory import AgentRunner


@dataclass
class PobiMetricEval:
...
global_metrics = PobiMetricEval()

F = TypeVar("F", bound=Callable[..., Awaitable[Any]])


def usage_tracking_decorator(
    metrics: PobiMetricEval | None = None,
) -> Callable[[F], F]:
...
def _wrap_run(cls: type, metrics: PobiMetricEval) -> None:
    """Apply the usage decorator to cls.run if defined directly on that class."""
    func = cls.__dict__.get("run")
    if func is None or getattr(func, "_pobi_instrumented", False):
        return

    decorated = usage_tracking_decorator(metrics)(func)
    setattr(decorated, "_pobi_instrumented", True)
    setattr(cls, "run", decorated)


def _wrap_hierarchy(root: type, metrics: PobiMetricEval) -> None:
...
def instrument_agent_runner(
    ...
    if getattr(AgentRunner, "_pobi_original_init_subclass", None):
        return None
...
def metrics_to_markdown(
    ...
    sections: list[str] = ["# Pobi Evaluation Report"]
