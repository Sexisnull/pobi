...
import { PobiRpcClient } from "../runtime/pobi-rpc-client.ts";
import type {
  AgentEvent,
...
export class PobiApp {
  private readonly root: BoxRenderable;
  private currentScreen: ScreenMode = "init";
  private rpcClient: PobiRpcClient | null = null;
  ...
  destroyAndExit(): void {
    ...
    console.log("\n  Leaving the pobi. Stay sharp out there.\n");
  }
  ...
  private bootstrap = async (): Promise<void> => {
    try {
      ...
      this.rpcClient = new PobiRpcClient(this.rpcLaunchConfig);
  ...
  private renderTranscript(): void {
    ...
    this.chatView.transcript.add(
      new TextRenderable(this.renderer, {
        content: "pobi CLI v0.1.5 · straylabs.ai",
        fg: theme.textSecondary,
      }),
    );
