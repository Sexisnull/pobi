...
export function getDefaultAppDirPath(): string {
  ...
  if (process.env.XDG_CONFIG_HOME) {
    return join(process.env.XDG_CONFIG_HOME, "pobi");
  }

  const homeDir = process.env.HOME ?? process.env.USERPROFILE;
  if (homeDir) {
    return join(homeDir, ".pobi");
  }

  return join(tmpdir(), "pobi");
}

export function getDefaultCacheDirPath(): string {
  if (process.env.XDG_CACHE_HOME) {
    return join(process.env.XDG_CACHE_HOME, "pobi");
  }

  const homeDir = process.env.HOME ?? process.env.USERPROFILE;
  if (homeDir) {
    return join(homeDir, ".cache", "pobi");
  }

  return join(tmpdir(), "pobi-cache");
}
...
async function resolveWritableAppDir(): Promise<string> {
  const candidates = uniquePaths([
    ...
    process.env.XDG_CONFIG_HOME ? join(process.env.XDG_CONFIG_HOME, "pobi") : undefined,
  ...
  throw new Error(
    `Unable to find a writable pobi state directory. Tried: ${candidates.join(", ")}`,
  );
}

async function resolveWritableCacheDir(): Promise<string> {
  const candidates = uniquePaths([
    process.env.XDG_CACHE_HOME ? join(process.env.XDG_CACHE_HOME, "pobi") : undefined,
    getDefaultCacheDirPath(),
    join(tmpdir(), "pobi-cache"),
  ]);
  ...
  throw new Error(
    `Unable to find a writable pobi cache directory. Tried: ${candidates.join(", ")}`,
  );
}
